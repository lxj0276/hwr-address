...
hcl = input_data(['省','市','县','区','乡','镇','村','巷','弄','X'], 20, 20, True, True, (28, 28), False)
...
#test result
batch = hcl.test.next_batch(200)
print 'test accuracy %g' % accuracy.eval(feed_dict={x:batch[0], y_:batch[1], keep_prob:1.0})

save_path = '/mnt/hgfs/3A/python/ml/hwr_pj/data_set/result/saveX.bin'
...
#print 'test accuracy %g' % accuracy.eval(feed_dict={x:batch[0], y_:batch[1], keep_prob:1.0})
print dir(y_conv)
yy = y_conv.eval(feed_dict={x:batch[0], y_:batch[1], keep_prob:1.0})
print yy
print len(yy)
print len(yy[0])
print yy[0]